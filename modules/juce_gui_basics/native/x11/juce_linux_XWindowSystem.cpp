...
void XWindowSystem::handleWheelEvent (LinuxComponentPeer* peer, const XButtonPressedEvent& buttonPressEvent, float amountX, float amountY) const
{
    MouseWheelDetails wheel;
    wheel.deltaX = amountX;
    wheel.deltaY = amountY;
...
void XWindowSystem::handleButtonPressEvent (LinuxComponentPeer* peer, const XButtonPressedEvent& buttonPressEvent) const
{
    ...
    if (mapIndex < (uint32) numElementsInArray (pointerMap))
    {
        ...
        {
            case Keys::WheelUp:         handleWheelEvent (peer, buttonPressEvent, 0.f, 50.0f / 256.0f); break;
            case Keys::WheelDown:       handleWheelEvent (peer, buttonPressEvent, 0.f, -50.0f / 256.0f); break;
            case Keys::WheelLeft:       handleWheelEvent (peer, buttonPressEvent, 50.0f / 256.0f, 0.f); break;
            case Keys::WheelRight:      handleWheelEvent (peer, buttonPressEvent, -50.0f / 256.0f, 0.f); break;
...
void XWindowSystem::handleButtonReleaseEvent (LinuxComponentPeer* peer, const XButtonReleasedEvent& buttonRelEvent) const
{
    ...
    if (mapIndex < (uint32) numElementsInArray (pointerMap))
    {
        switch (pointerMap[mapIndex])
        {
            ...
            case Keys::BackButton:      ModifierKeys::currentModifiers = ModifierKeys::currentModifiers.withoutFlags (ModifierKeys::backButtonModifier);    break;
            case Keys::ForwardButton:   ModifierKeys::currentModifiers = ModifierKeys::currentModifiers.withoutFlags (ModifierKeys::forwardButtonModifier); break;
            default: break;
        }
    }
